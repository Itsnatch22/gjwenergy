  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-obsidian/90 backdrop-blur-md" data-testid="navbar">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6 md:h-20 lg:px-10">
        <Link href="/" className="flex items-center" data-testid="nav-logo" onClick={() => setOpen(false)}>
          ...
          {LINKS.map((l) => (
            <Link
              ...
              className={`font-mono text-xs uppercase tracking-[0.2em] transition-colors duration-300 ${
                pathname === l.to ? "text-ochre" : "text-white/60 hover:text-white"
              }`}
            >
              {l.label}
            </Link>
            ...
            <div className="flex flex-col gap-1 px-6 py-6">
              {LINKS.map((l) => (
                <Link
                  ...
                  className={`py-3 font-display text-2xl font-bold uppercase tracking-tight transition-colors duration-300 ${
                    pathname === l.to ? "text-ochre" : "text-white/70 hover:text-white"
                  }`}
                >
                  {l.label}
                </Link>
